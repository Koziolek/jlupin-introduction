...
function DiscoverMicroservices()
  ...
  do
    ...
    local previousMicroservicePort = CommonGetMicroserviceProperty(name,"directPort")
    if not previousMicroservicePort
    then
      previousMicroservicePort = 0
    end
    ...
    local currentMicroserviceRestart = RegGet("nodeInfo","__" .. name .. "_restart")
    if not currentMicroserviceRestart
    then
    ...
    -- helper for managing proces restart
    RegSet("nodeInfo","__" .. name .. "_restart",currentMicroserviceRestart,discoverTimerLockTimeout)
    ...
    ngx.log(ngx.DEBUG,"[JLUPIN DISCOVER] Worker " .. ngx.worker.pid() .. ": Microservice: " .. name .. ": END(microserviceList): " .. GenTableToJson(microserviceList))
  end
