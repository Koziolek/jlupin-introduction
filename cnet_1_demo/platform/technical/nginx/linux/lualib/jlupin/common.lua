...
-----------------------------------
function CommonGetMicroserviceProperty(microservice_name,microservice_property)

  ngx.log(ngx.DEBUG,"[JLUPIN] Worker " .. ngx.worker.pid() .. ": BEGIN(" .. GenNilHandler(microservice_name) .. "," .. GenNilHandler(microservice_property).. ")")

  if not microservice_name
  then
    ngx.log(ngx.ERR,"[JLUPIN] Worker " .. ngx.worker.pid() .. ": microservice_name is nil")
    return nil,"microservice_name is nil"
  end

  if not microservice_property
  then
    ngx.log(ngx.ERR,"[JLUPIN] Worker " .. ngx.worker.pid() .. ": microservice_property is nil")
    return nil,"microservice_property is nil"
  end
  ...
  then
    ...
    return nil,"microserviceList is nil"
  end
